...
async function getAllItems(name, select) {
  ...
  while (url) {
    ...
    if (!res.ok) {
      if (res.status === 404) throw new Error(`List "${listName(name)}" was not found. Run provision-via-sitedesign.ps1 on this site first.`);
...
export async function probeSharePoint() {
  ...
  if (res.status === 404) throw new Error('SharePoint lists are not provisioned on this site yet. Run provision-via-sitedesign.ps1 first.');
