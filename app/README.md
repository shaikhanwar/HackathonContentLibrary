...
```
app/index.html          ->  SiteAssets/hcl/index.html
...
- **`spconfig.js`** treats any `*.sharepoint.com` host as SharePoint mode and derives the site URL from the path (trims at `/SiteAssets/`), so a static `index.html` (which gets no `_spPageContextInfo`) still targets `…/sites/HackathonContentLibrary` correctly.

## Keeping this in sync
This folder is a copy of `../../prototype/{index.html,css,js}`. If those source files change, re-copy them here before re-uploading.
